...
function alloysApplyPreset(mIdx, preset) { minerals[mIdx].coefficients = preset === 'A' ? [...PRESET_ALLOY_A] : [...PRESET_ALLOY_B]; alloysRender(); }
...
async function alloysCalculate() {
     ...
     * @returns
     */
    function combinations_per_coeff(bounds, callback, current_item_usage = [], index = 0) {
    ...
    let valid_axis_data_from_minerals = {};
    for (const mineral of Object.values(minerals)) {
        ...
        combinations_per_coeff(mineral.item_bounds, (current_item_count_the_coeffs_uses) => {
            /*
            ...
            */
            ...
            if (total_item_count > MAX_TOTAL_ITEM)
                return;
            ...
            if ((sum_mb * 100) / MAX_VESSEL_MB > mineral.ratioMax)
                return;
            ...
            if (total_slots_usage >= MAX_SLOTS)
                return;
            if (total_slots_usage > MAX_SLOTS - (minerals.length - 1))
                return;
            let point_data = {
    ...
    let point_data_state;
    (function (point_data_state) {
        point_data_state[point_data_state["slots_exceeded"] = 0] = "slots_exceeded";
        point_data_state[point_data_state["items_exceeded"] = 1] = "items_exceeded";
        point_data_state[point_data_state["milibuckets_exceeded"] = 2] = "milibuckets_exceeded";
        point_data_state[point_data_state["milibuckets_leftover"] = 3] = "milibuckets_leftover";
        point_data_state[point_data_state["invalid_ratio"] = 4] = "invalid_ratio";
        point_data_state[point_data_state["valid"] = 5] = "valid";
    })(point_data_state || (point_data_state = {}));
    /**
     ...
     * @returns
     */
    function cartesian_product(bounds, current_point = [], index = 0) {
        if (index == bounds.length) {
            let current_slot_usage_from_all_axis = 0;
            let current_item_usage_from_all_axis = 0;
            let current_milibucket_from_all_axis = 0;
            let weighted_sum_mb_from_each_axis = [];
            let item_count_used_in_variables = [];
            let minerals_detail = [];
            ...
            if (total_slot_usage_from_all_axis > MAX_SLOTS)
                return point_data_state.slots_exceeded;
            if (total_item_usage_from_all_axis > MAX_TOTAL_ITEM)
                return point_data_state.items_exceeded;
            if (total_milibucket_usage_from_all_axis > MAX_VESSEL_MB)
                return point_data_state.milibuckets_exceeded;
            if (total_milibucket_usage_from_all_axis % MB_TO_INGOT != 0)
                return point_data_state.milibuckets_leftover;
            for (let axis_ws = 0; axis_ws < weighted_sum_mb_from_each_axis.length; axis_ws++) {
                ...
                if (metal_percent < minerals[axis_ws].ratioMin || metal_percent > minerals[axis_ws].ratioMax)
                    return point_data_state.invalid_ratio;
            }
        ...
        for (let i = 0; i < bounds[index]; i++) {
            ...
            if (state === point_data_state.slots_exceeded)
                break; // 110ms
        }
    }
...
function buildHighlightDetail(item) {
    ...
    item.minerals.forEach((md, mIdx) => {
        ...
        const varParts = md.items_from_variable.map((v, ci) => {
  ...
  `;
    ...
    results.forEach((r, i) => {
        ...
        r.minerals.forEach((md, mIdx) => {
            const color = minerals[mIdx] ? minerals[mIdx].color : 'inherit';
            md.items_from_variable.forEach((v, ci) => {
                const isFirst = ci === 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('alloys-minerals')?.addEventListener('change', (event) => {
        ...
        switch (target.dataset.action) {
            case 'update-name':
                alloysUpdateName(target);
                break;
            case 'update-color':
                alloysUpdateColor(target);
                break;
            case 'update-ratio':
                alloysUpdateRatio(target);
                break;
            case 'update-coeff':
                alloysUpdateCoeff(target);
                break;
        }
    });
    document.getElementById('alloys-minerals')?.addEventListener('click', (event) => {
        ...
        switch (target.dataset.action) {
            case 'remove-mineral':
                alloysRemoveMineral(mineralIndex);
                break;
            case 'remove-coeff':
                alloysRemoveCoeff(mineralIndex, coefficientIndex);
                break;
            case 'add-coeff':
                alloysAddCoeff(mineralIndex);
                break;
            case 'preset-a':
                alloysApplyPreset(mineralIndex, 'A');
                break;
            case 'preset-b':
                alloysApplyPreset(mineralIndex, 'B');
                break;
        }
    });
});
