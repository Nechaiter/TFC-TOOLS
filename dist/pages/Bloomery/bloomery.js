...
async function performBloomeryCalculations() {
    ...
    cartesian_product(ironCoefficients, (solution) => {
        const sum_vars = solution.reduce((a, b) => a + b, 0);
        if (sum_vars >= MAX_TOTAL_ITEMS)
            return;
        if (sum_vars === 0)
            return;
        const sum_mb = solution.reduce((a, b, i) => a + b * ironCoefficients[i], 0);
...
function buildBloomeryDetail(item) {
    ...
    const varParts = item.var_counts
        ...
        if (v === 0)
            return '';
        const cLabel = ironCoefficients[ci] !== undefined
            ? ironCoefficients[ci]
            : '?';
        return `
                <span class="best-var">
...
function renderBloomeryResults(results) {
    ...
    console.log(best);
    const bestIngots = document.getElementById('best-ingots');
    const bestOre = document.getElementById('best-ore');
    const bestMb = document.getElementById('best-mb');
    const bestCoal = document.getElementById('best-coal');
    ...
    const pointsCount = document.getElementById('bloomery-points-count');
    if (pointsCount) {
...
function toggleBloomeryTable() {
    const toggle = document.getElementById('bloomery-table-toggle');
    const body = document.getElementById('bloomery-table-body');
    if (toggle) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('mb-preset-144')?.addEventListener('click', () => {
        document.getElementById('mb-per-ingot').value = '144';
    });
    document.getElementById('mb-preset-100')?.addEventListener('click', () => {
        document.getElementById('mb-per-ingot').value = '100';
    });
    ...
    document.getElementById('iron-coeff-list')?.addEventListener('click', (event) => {
        const target = event.target.closest('[data-action="remove-coeff"]');
        if (target)
            bloomeryRemoveCoeff(parseInt(target.dataset.cidx ?? '', 10));
    });
});
export {};
//# sourceMappingURL=bloomery.js.map